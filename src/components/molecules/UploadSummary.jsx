...
const totalSize = files.reduce((acc, file) => acc + file.size, 0)
  ...
  const totalSavings = totalSize - compressedTotalSize
  const compressionRatio = totalSize > 0 ? Math.round((totalSavings / totalSize) * 100) : 0
  ...
  return (
    ...
    <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
            {pendingFiles > 0 && <Button variant="primary" size="sm" onClick={onUploadAll}>
                <ApperIcon name="Upload" size={16} className="mr-2" />Upload All ({pendingFiles})
                            </Button>}
            <Button
                variant="ghost"
                size="sm"
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        {totalSavings > 0 && <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{compressionRatio}%</div>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
            <Badge variant="default" icon="Files">
                {formatFileSize(totalSize)}original
                        </Badge>
            {totalSavings > 0 && <Badge variant="primary" icon="Zap">
                {formatFileSize(compressedTotalSize)}compressed
                          </Badge>}
            {completedFiles > 0 && <Badge variant="success" icon="CheckCircle">
                {completedFiles}completed
                          </Badge>}
            {uploadingFiles > 0 && <Badge variant="primary" icon="Upload">
                {uploadingFiles}uploading
                          </Badge>}
            {pendingFiles > 0 && <Badge variant="warning" icon="Clock">
                {pendingFiles}pending
                          </Badge>}
            {failedFiles > 0 && <Badge variant="error" icon="XCircle">
                {failedFiles}failed
                          </Badge>}
            {totalSavings > 0 && <Badge variant="success" icon="TrendingDown">
                {formatFileSize(totalSavings)}saved
                          </Badge>}
            {/* Progress Overview */}
            {totalFiles > 0 && <div className="bg-gray-50 rounded-lg p-4">
